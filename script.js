...
window.onload = async function () {
    ...
    try {
        ...
        if (ingredientsData.length === 0 || allRecipes.length === 0) {
            loadingMessage.textContent = 'Для работы приложения загрузите ingredients.xlsx и recipes.xlsx вручную.';
        ...
        loadingMessage.textContent = `Ошибка загрузки данных: ${error.message}.`;
...
function handleIngredientFile(event) {
    ...
    reader.onload = function(e) {
        ...
        saveToIndexedDB('ingredients', ingredientsData);
        alert(`Файл ${file.name} успешно загружен как ингредиенты.`);
        updateRecipeList();
    };
    reader.readAsArrayBuffer(file);
}

function handleRecipeFile(event) {
    const file = event.target.files[0];
    if (!file) return;
    ...
    reader.onload = function(e) {
        ...
        alert(`Файл ${file.name} успешно загружен как рецепты.`);
        updateRecipeList();
    };
    reader.readAsArrayBuffer(file);
}
